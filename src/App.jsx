...
function load() {
  try {
    ...
    if (r) {
      ...
      return {
        ...def, ...parsed,
        routine: { ...def.routine, ...(parsed.routine||{}) },
        fitness: { ...def.fitness, ...(parsed.fitness||{}) },
        diet: { ...def.diet, ...(parsed.diet||{}) },
        finance: { ...def.finance, ...(parsed.finance||{}) },
        wellbeing: { ...def.wellbeing, ...(parsed.wellbeing||{}) },
...
function SectionTitle({ icon, title, sub, onReset }) {
  return (
    <div style={{ marginBottom: 14, display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
      ...
      {onReset && <div style={{ marginTop:4 }}><ResetButton onReset={onReset} /></div>}
    </div>
  );
}
...
const QUOTES = [
  ...
  "Growth is quiet before it's loud.",
...
function dailyQuote() {
  // Deterministic pick based on the date, so it changes once per day
  let hash = 0;
...
// ── Reset control ────────────────────────────────────────────────────────────
function ResetButton({ onReset, label="Reset this section" }) {
  const [confirming, setConfirming] = useState(false);

  if (confirming) {
    return (
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        <span style={{ fontFamily:FB, fontSize:12, color:C.textDim }}>Sure?</span>
        <span onClick={()=>{ onReset(); setConfirming(false); }} style={{
          fontFamily:FB, fontSize:12, fontWeight:700, color:C.red, cursor:"pointer"
        }}>Yes, reset</span>
        <span onClick={()=>setConfirming(false)} style={{
          fontFamily:FB, fontSize:12, color:C.textMut, cursor:"pointer"
        }}>Cancel</span>
      </div>
    );
  }
  ...
  return (
    <div onClick={()=>setConfirming(true)} title={label} style={{
      fontSize:16, cursor:"pointer", color:C.textMut, lineHeight:1
    }}>↺</div>
  );
}
...
function Home({ data, setActive }) {
  ...
  return (
    <div>
      <Card style={{ background: C.blush, border:`1px solid ${C.pink}33` }}>
        <div style={{ fontFamily:F, fontSize:15, fontWeight:700, color:C.text, fontStyle:"italic", lineHeight:1.4 }}>
          "{dailyQuote()}"
        </div>
      </Card>
...
function Routine({ data, update }) {
  ...
  const resetPeriod = (period) => {
    const list = data.routine[period].map(x => ({...x, done:false}));
    update({ ...data, routine: { ...data.routine, [period]: list } });
  };

  return (
    <div>
      <SectionTitle icon="☀️" title="Morning Routine" onReset={()=>resetPeriod("morning")} />
      <Card>
      ...
      <SectionTitle icon="🌙" title="Evening Routine" onReset={()=>resetPeriod("evening")} />
      <Card>
...
function Tasks({ data, update }) {
  ...
  const resetTasks = () => update({ ...data, tasks: [] });

  return (
    <div>
      <SectionTitle icon="✅" title="Daily Tasks" sub={`${doneCount}/${data.tasks.length} done`} onReset={resetTasks} />
      <Card>
...
function Faith({ data, update }) {
  ...
  const resetPrayers = () => update({ ...data, prayers: { ...data.prayers, logs: { ...data.prayers.logs, [TODAY]: {} } } });
  const resetQuran = () => update({ ...data, quran: { ...data.quran, logs: { ...data.quran.logs, [TODAY]: false } } });

  return (
    <div>
      <SectionTitle icon="🕌" title="Prayers" sub={`${doneCount}/5 prayed today`} onReset={resetPrayers} />
      <Card>
        {PRAYERS.map(p => (
      ...
      <SectionTitle icon="📖" title="Qur'an" onReset={resetQuran} />
      <Card>
...
function Fitness({ data, update }) {
  ...
  const weightEntries = data.fitness.weightEntries || [];
  ...
  const logWeight = () => {
    if (!weightVal) return;
    const newEntries = [...weightEntries, { date:TODAY, value: parseFloat(weightVal) }];
    update({ ...data, fitness: { ...data.fitness, weightEntries: newEntries } });
    setWeightVal("");
  };

  const resetActivity = () => update({ ...data, fitness: { ...data.fitness, logs: [] } });
  const resetSleep = () => update({ ...data, fitness: { ...data.fitness, sleepLogs: [] } });
  const resetWeight = () => update({ ...data, fitness: { ...data.fitness, weightEntries: [] } });

  return (
    <div>
      <SectionTitle icon="💪" title="Fitness" onReset={resetActivity} />
      <Card>
      ...
      <SectionTitle icon="😴" title="Sleep" sub="Log your bedtime and wake time" onReset={resetSleep} />
      <Card>
      ...
      <SectionTitle icon="⚖️" title="Weight tracking" sub="Suivi de poids" onReset={resetWeight} />
      <Card>
        <div style={{ display:"flex", gap:8, marginBottom:12 }}>
        ...
        {weightEntries.length > 1 && (
          <div>
            {weightEntries.slice().reverse().slice(0,5).map((w,i)=>(
...
function Diet({ data, update }) {
  ...
  const todayMeals = (data.diet.mealLogs && data.diet.mealLogs[TODAY]) || [];
  ...
  const resetWater = () => update({ ...data, diet: { ...data.diet, waterLogs: { ...data.diet.waterLogs, [TODAY]: 0 } } });
  const resetMeals = () => update({ ...data, diet: { ...data.diet, mealLogs: { ...(data.diet.mealLogs||{}), [TODAY]: [] } } });

  return (
    <div>
      <SectionTitle icon="💧" title="Water Intake" sub="Glasses today" onReset={resetWater} />
      <Card>
      ...
      <SectionTitle icon="🥗" title="What did you eat today?" onReset={resetMeals} />
      <Card>
        <div style={{ display:"flex", gap:8, marginBottom:12 }}>
...
function Learning({ data, update }) {
  ...
  const resetBooks = () => update({ ...data, learning: { ...data.learning, books: [] } });

  return (
    <div>
      <SectionTitle icon="📚" title="Reading list" onReset={resetBooks} />
      <Card>
        <div style={{ display:"flex", gap:8, marginBottom:12 }}>
...
function Goals({ data, update }) {
  ...
  const resetGoals = () => update({ ...data, goals: [] });

  return (
    <div>
      <SectionTitle icon="🎯" title="Goals" onReset={resetGoals} />
      <Card>
        <div style={{ display:"flex", gap:8, marginBottom:12 }}>
...
function Finance({ data, update }) {
  ...
  const resetExpenses = () => update({ ...data, finance: { ...data.finance, expenses: [] } });
  const resetSavings = () => update({ ...data, finance: { ...data.finance, savingsGoals: [] } });
  const resetDebts = () => update({ ...data, finance: { ...data.finance, debts: [] } });
  ...
  return (
    <div>
      ...
      <SectionTitle icon="💸" title="Expenses" sub={`Total: ${totalMonth.toFixed(2)}`} onReset={resetExpenses} />
      <Card>
        <div style={{ display:"flex", gap:8, marginBottom:12 }}>
      ...
      <SectionTitle icon="🏦" title="Savings goals" onReset={resetSavings} />
      <Card>
        <div style={{ display:"flex", gap:8, marginBottom:12 }}>
      ...
      <SectionTitle icon="📉" title="Debts" onReset={resetDebts} />
      <Card>
        <div style={{ display:"flex", gap:8, marginBottom:12 }}>
...
function Wellbeing({ data, update }) {
  ...
  const resetMood = () => update({ ...data, wellbeing: { ...data.wellbeing, moods: { ...data.wellbeing.moods, [TODAY]: undefined } } });
  const resetGratitude = () => {
    setGratitude("");
    update({ ...data, wellbeing: { ...data.wellbeing, gratitude: { ...data.wellbeing.gratitude, [TODAY]: "" } } });
  };

  return (
    <div>
      <SectionTitle icon="🌸" title="Mood" sub="How are you feeling today?" onReset={resetMood} />
      <Card>
      ...
      <SectionTitle icon="🙏" title="Gratitude" sub="What are you grateful for today?" onReset={resetGratitude} />
      <Card>
